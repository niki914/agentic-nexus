...
4. 读取 `WebSettings.packageName` 和 `WebSettings.config`
...
5. `BaseConfigProvider` 通过 `XService.getWebSettings(...).config` 提供点号分隔路径的层级取值（如 `classes.room_id_manager`）
6. `BreenoConfigProvider` 对每个 key 做类型化封装
