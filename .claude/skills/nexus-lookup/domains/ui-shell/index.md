...
- `NexusPage.kt`：定义 `StartupPage`、`ProviderPickPage`、`ConfigurePage`、`DonePage`、`HomePage`、`SettingsHomePage`、`SettingsDetailPage`、`McpServerDetailPage`、`CustomToolDetailPage`、`ExecutionRuleDetailPage`。
...
- `ExecutionRulesSettingsState.kt`：维护 ExecutionRules 列表页状态；当前加载后仍为空列表。
...
- 不要把 settings 全部视为已完成；`Memory` 与 `About` 已有真实内容，`ExecutionRules` 仍只有列表与详情壳，缺少真实规则数据源和编辑表单。
