...
- **`app/`** (application)：Xposed 模块的启动入口、宿主路由分发、主 App 的 Compose 壳层 UI，以及各语音助手具体的业务 Hook 实现。
- **`h/`** (library)：Xposed 框架层封装。负责 Runtime 编排、反射调用机制、Context 上下文捕获、日志记录与容错处理。
- **`composebase/`** (library)：为 UI 层提供 Compose 主题配置与 ViewModel 基础设施。
...
- **Xposed 与业务入口**
  ...
  - LLM 请求调度控制器：`app/src/main/java/com/niki914/nexus/agentic/chat/LLMController.kt`
