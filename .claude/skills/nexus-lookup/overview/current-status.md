...
- **设置页真实内容**：`ModelConfig`、`Memory`、`BuiltinTools`、`CustomShellTools`、`Mcp`、`About` 已有独立内容；`Memory` 接入 `XRepo.memory` 读写，`McpServerDetailContent` 与 `CustomToolDetailContent` 已是可编辑详情页。
...
- **ExecutionRules 设置页**：`ExecutionRules` 已有列表页、启用切换模型和详情页路由，但数据源仍是空列表，详情内容仍未接入真实规则编辑。
- **命令执行安全**：`run_command` 与 custom tool 都有基础安全策略和超时控制，但当前仍是基础黑名单拦截，不是完整沙箱或审批体系。

## 未见实现

- **独立的宿主差异化启动页**：当前没有针对 `Breeno` / `XiaoAi` 的独立 startup 页面实现。
- **ExecutionRules 真实规则数据源与详情编辑**：当前源码里未见规则持久化来源或可编辑详情表单。
