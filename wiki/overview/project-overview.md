...
- `store/`：多 store 持久化实现；当前静态 store 包括 `settings/hooks.json`、`settings/agents/main/config.json`、`settings/agents/main/memory.json`、`settings/tools/builtin_tools.json`、`settings/tools/custom_tools.json`、`settings/tools/mcp/servers.json`、`settings/rules/execution_rules.json`、`settings/rules/takeover_rules.json`、`settings/app_state.json`，并按 MCP server 动态扩展 `settings/tools/mcp/cache/<serverId>.json`。

## 设置模型

- hook 配置仍通过 `web_settings` 语义读取，但文件路径已落到 `settings/hooks.json`。
- 面向 agent 的 LLM、memory、builtin、custom、MCP、execution、takeover、app state 已拆到独立 store，由 `app/src/main/java/com/niki914/nexus/agentic/repo/XRepo.kt` 聚合访问。
